...
class IsaacPolicyRunner(Node):
    def __init__(self) -> None:
        ...
        self.debug_print = (
            self.get_parameter("debug_print").get_parameter_value().bool_value
        )
        self.debug_every_n = (
            int(self.get_parameter("debug_every_n").get_parameter_value().integer_value)
            or 1
        )
        self._debug_count = 0
        ...
        self.use_internal_action_observation = (
            self.get_parameter("use_internal_action_observation")
            .get_parameter_value()
            .bool_value
        )
        ...
        try:
            self.get_logger().info(
                "IO action joint order: " + ", ".join(self.io.joint.action_joint_names)
            )
            self.get_logger().info(
                "IO observation joint_pos_rel order: "
                + ", ".join(self.io.joint.joint_names)
            )
        ...
        self.sub = self.create_subscription(
            Float32MultiArray, "/policy/observations", self._cb_obs, 10
        )
    ...
    def _cb_obs(self, msg: Float32MultiArray) -> None:
        ...
        if obs.size != self.obs_size:
            self.get_logger().warning(
                f"Observation size mismatch: got {obs.size}, expected {self.obs_size}"
            )
            return
        ...
        if self.debug_print:
            ...
            if (self._debug_count % self.debug_every_n) == 0:
                ...
                generated_commands = _sl("generated_commands")
                joint_pos_rel = _sl("joint_pos_rel")
                joint_vel_rel = _sl("joint_vel_rel")
                last_action = _sl("last_action")
                self.get_logger().info(f"[{now}] [policy] observations: {obs}")
                if base_lin_vel is not None:
                    self.get_logger().info(f"  base_lin_vel: {base_lin_vel}")
                if base_ang_vel is not None:
                    self.get_logger().info(f"  base_ang_vel: {base_ang_vel}")
                if projected_gravity is not None:
                    self.get_logger().info(f"  projected_gravity: {projected_gravity}")
                if generated_commands is not None:
                    self.get_logger().info(
                        f"  generated_commands: {generated_commands}"
                    )
                if joint_pos_rel is not None:
                    self.get_logger().info(f"  joint_pos_rel: {joint_pos_rel}")
                if joint_vel_rel is not None:
                    self.get_logger().info(f"  joint_vel_rel: {joint_vel_rel}")
                if last_action is not None:
                    self.get_logger().info(f"  last_action: {last_action}")
        ...
        if act.size != self.act_size:
            self.get_logger().warning(
                f"Action size mismatch: got {act.size}, expected {self.act_size}"
            )
            return

        if self.debug_print and (self._debug_count % self.debug_every_n) == 0:
            now = datetime.datetime.now().strftime("%H:%M:%S.%f")
            self.get_logger().info(f"[{now}] [policy] actions: {act}")
